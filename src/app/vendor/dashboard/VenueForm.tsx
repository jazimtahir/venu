...
import { createVenue, updateVenue } from '@/app/actions/venue';
...
export function VenueForm({ vendorId, venue }: VenueFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isEdit && venue) {
      ...
      router.push(`/vendor/dashboard/venues/${result.id}/edit?created=1`);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-1.5">Venue type *</label>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-1.5">Total capacity</label>
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label htmlFor="catering" className="text-sm font-medium text-foreground cursor-pointer">Catering included</label>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Per head with catering – min (PKR)</label>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Per head with catering – max (PKR)</label>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Per head without catering – min (PKR)</label>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Per head without catering – max (PKR)</label>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-1.5">Google Maps link</label>
      ...
      <button
        type="submit"
        disabled={loading}
        className="rounded bg-brand px-6 py-2.5 text-sm font-medium text-[var(--page-bg)] hover:bg-brand-hover disabled:opacity-50 transition-colors duration-200"
      >
